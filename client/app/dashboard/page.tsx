...
export default async function DashboardPage() {
  ...
  const predRes = await fetch(`${API}/api/prediction`, {
    cache: "no-store",
  });
  if (!predRes.ok) throw new Error("Failed to fetch prediction");
  const prediction: Prediction = await predRes.json();
    ...
    <main className="mx-auto max-w-5xl space-y-8 p-6">
      ...
      <section className="grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-xl border bg-purple-50 p-4 shadow-sm">
          <h2 className="flex items-center text-lg text-black font-semibold">
            Energy Prediction
          ...
          <p className="mt-2 text-sm text-black">
            Next hour usage:{" "}
            <strong>{prediction.predictedEnergyKWh} kWh</strong>
          </p>
      ...
      <section className="space-y-4">
        ...
        {rooms.map((room) => {
          ...
          return (
            ...
            >
              <h3 className="mb-3 text-lg font-semibold text-black">{room.name}</h3>
              ...
              <div className="space-y-2">
                {roomDevices.map((device) => (
                  <div
                    key={device.id}
                    className="flex items-center justify-between rounded-md text-black bg-gray-50 px-3 py-2 text-sm"
                  >
      ...
      <section className="rounded-xl border bg-gray-50 p-4 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-600">
          The AI agent continuously evaluates room occupancy,
          temperature drift, weather context, and predicted energy
          consumption. Based on these signals, it proactively turns
          devices ON or OFF and adjusts settings to minimize energy
